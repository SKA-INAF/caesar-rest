...
def background_task(self,app_name,cmd,cmd_args,job_top_dir,username='anonymous',db_host='localhost', db_port='27017', db_name='caesardb',monitoring_period=10):
	"""Background task """
	...
	try:
		...
		update_celery_task_state(self,'ABORTED',task_info)
		return res
	...
	update_celery_task_state(self,'PENDING',task_info)
	...
	update_celery_task_state(self,'STARTED',task_info)
		...
		try:
			while True:
				...
				if p.poll() is None:
					...
					update_celery_task_state(self,'RUNNING',task_info)
			...
			update_celery_task_state(self,'TIMED-OUT',task_info)
	...
	update_celery_task_state(self,task_state,task_info)
...
#######################################
####   SUBMIT BATCH TASK TO SLURM
#######################################
@celery_app.task(bind=True)
def batch_task(self, app_name, cmd, cmd_args, job_top_dir, username='anonymous', db_host='localhost', db_port='27017', db_name='caesardb'):
	""" Task submitting jb to Slurm batch system """
	...
	submit_batch_job(
		app_name, task_id,
		cmd, cmd_args, job_dir,
    client, db_name, username
	)
	# ...
	# ...
...
def submit_batch_job(app_name, task_id, cmd, cmd_args, job_dir, client, db_name, username):
	""" Submit a task to Slurm batch system """
	...
	genscript_tinfo= run_local_task(cmd, cmd_args, job_dir)

	if genscript_tinfo['state']!="SUCCESS":
		errmsg= "Submission script generation failed (err=" + genscript_tinfo['status'] + ")"
		logger.error(errmsg)
		res['msg']= errmsg
		return res

	# - Retrieve name of submission script
	# ...

	# - Read submission script line by line and put it in a string variable separated by \n (needed by slurm rest)
	# ...

	# - Submit script to Slurm cluster through rest API
	# ...
...
####     RUN LOCAL TASK
#######################################
def run_local_task(cmd, cmd_args, job_dir, monitoring_period=5):
	""" Run a task locally"""
	...
	# - Execute command
	logger.info("Executing background task (cmd %s, args %s) ..." % (cmd,cmd_args))
	exec_cmd= ' '.join([cmd,cmd_args])
	...
	logger.info("Background task started with pid=%s ..." % str(pid))
	start = time.time()
	...
	try:
		try:
			...
			task_info['status']= status_msg
			task_info['elapsed_time']= elapsed
			task_info['exit_code']= 124 # equivalent sigterm
		...
		logger.info("Task monitoring interrupted with ctrl-c signal")
		task_info['state']= 'ABORTED'
		task_info['status']= "Task monitoring interrupted with ctrl-c signal"
		task_info['elapsed_time']= elapsed
		task_info['exit_code']= 124 # equivalent sigterm
		return task_info
		#raise Ignore()
...
####    UPDATE JOB STATUS
#######################################
def update_celery_task_state(task_instance, state, task_info):
	""" Update celery task state """

	# - Check task instance
	if task_instance is None:
		logger.error("Given task instance is None!")
		return -1

	# - Update task, catching possible exceptions
	try:
		task_instance.update_state(state=state, meta=task_info)
	
	except Exception as e:
		errmsg= 'Exception caught when updating celery task state (err=' + str(e) + ')!' 
		logger.error(errmsg)
		return -1
